...
def next_step(qnow, qs, encoder_params_list, height, sigma, dt=1e-3, beta=1.0):
    if qs is None:
        ...
        qnext = qnow + (- (gradV(qnow) + GradGaussian(qnow, qs, encoder_params_list, height, sigma))) * dt + np.sqrt(2 * dt / beta) * np.random.randn(*qnow.shape)
    return qnext
...
if __name__ == '__main__':
    ...
    Gs = JSumGaussian(np.concatenate([X.reshape(-1, 1), Y.reshape(-1, 1), np.zeros((num_points, n))], axis=1), qs, encoder_params_list, h=height, sigma=sigma)
