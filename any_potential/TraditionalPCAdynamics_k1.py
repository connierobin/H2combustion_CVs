...
def trad_replacement(data):

    # eigenvectors = jnp.identity(2+n)
    eigenvectors = jnp.zeros((2+n,2+n))
    eigenvectors = eigenvectors.at[0,0].set(1)
    eigenvectors = eigenvectors.at[1,0].set(-2)
    # print(f'eigenvectors here: {eigenvectors}')

    # selected_eigenvectors = jnp.ones((1,2+n))
    eigenvalues = jnp.std(data, axis=0)
    ...
    for i in tqdm(range(Nsteps)):
        ...
        if (i + 1) % NstepsDeposite == 0:
            ...
            if qs is None:
                ...
                # The x part is the "center" of the first eigenvector, the y part is the "center" of the second
                qs = mean_vector                #data[-2:-1]#mean_vector


                sigmas = eigenvalues * sigma_factor
                # qs = center_vector
                ...
                choose_eigenvalue = [np.zeros(2+n)]
                choose_eigenvalue[0][0] = 1.
                # choose_eigenvalue[0][1] = 1.
                choose_eigenvalue = jnp.array(choose_eigenvalue)


                # CHANGED to use all eigenvalues
                ...
                # CHANGED already calculated std's
                ...
                choose_eigenvalue_tmp = [np.zeros(2+n)]
                choose_eigenvalue_tmp[0][0] = 1.
                # choose_eigenvalue_tmp[0][1] = 1.
                choose_eigenvalue_tmp = np.array(choose_eigenvalue_tmp)
                choose_eigenvalue = np.concatenate([choose_eigenvalue, choose_eigenvalue_tmp], axis=0)
...
if __name__ == '__main__':
    ...
    for i in range(10):
        name = f'Trad_results/run_ws_n1_k1_{i+3}.h5'
