...
# This function and SumGaussiansPCA do the same thing
...
def main_plot(potential, n, trajectory, qs, eigenvectors_list, choose_eigenvalue_list, gradient_directions, sigma, decay_sigma, height, NstepsDeposite, T, threshold, timings):
    filename = None
    ...
    print(f'eigenvectors.shape: {eigenvectors_list.shape}')
    print(f'qs.shape: {qs.shape}')
    # I can't seem to figure out how to plot the PCA eigenvectors...
    for i in range(eigenvectors_list.shape[0]):
        x = qs[i, 0]
        y = qs[i, 1]
        pos_list_x = [x, x]
        pos_list_y = [y, y]
        # print(pos_list)
        print(eigenvectors_list[i, :, 0])
        ax2.quiver(pos_list_x, pos_list_y, eigenvectors_list[i, :, 0], eigenvectors_list[i, :, 1])
        # ax2.quiver([qs[:, 0], qs[:, 1]], eigenvectors_list[:, 1, 0], eigenvectors_list[:, 1, 1])
...
def load_data(filename):
    ...
    main_plot(pot_fn, parameters['n'], trajectory, qs, eigenvectors, choose_eigenvalue, gradient_directions, parameters['sigma'], parameters['decay_sigma'], parameters['height'], NstepsDeposite, parameters['T'], parameters['threshold'], timings)
...
if __name__ == '__main__':
    ...
    results = []
    for i in range(1):
        filename = f'../PCA_results/run_test_{i+6}.h5'
