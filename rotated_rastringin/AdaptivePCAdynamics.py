...
def potential(qx, qy, qn, pot_name):
    ...
    elif pot_name == 'rastringin' or pot_name == 'rastrigin':
        return rastringin_potential(qx, qy, qn)
    return rotated_rastringin_potential(qx, qy, qn)
...
def gradV(q, pot_name):
    if pot_name == 'rosenbrock':
        ...
        return grad_rastringin(q)
    return grad_rotated_rastringin(q)
...
def grad_rastringin(q):
    A = 10
    B = 0.5
    V = B*2*q + A*jnp.sin(2*jnp.pi*q)*2*jnp.pi
    return -V
...
def rotated_rastringin_potential(q, angle=(np.pi / 4)):
    need_swap = False
    if q.ndim > 1:
        need_swap = True
...
def grad_rotated_rastringin(q, angle=(np.pi / 4)):
    need_swap = False
    if q.ndim > 1:
        need_swap = True
        # Move the last dimension to the first position
    ...
    for i in tqdm(range(Nsteps)):
        ...
        if (i + 1) % NstepsDeposite == 0:
            ...
            if qs is None:
                ...
                # print(f'choose_eigenvalue_tmp: {choose_eigenvalue_tmp}')
...
def run(filename=None, T=4, potential='wolfeschlegel'):
    ...
    max_qn_val = 2     # size of the relevant part of the potential surface

    q0 = np.concatenate((np.array([[0.0, 0.0]]), np.array([np.random.rand(n)*(2*max_qn_val) - max_qn_val])), axis=1)
...
if __name__ == '__main__':
    ...
    potential='rotated_rastringin'

    for i in range(10):
        name = f'PCA_results/run_rotras_n48_{i+41}.h5'
