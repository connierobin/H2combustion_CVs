...
def visualize_cn(irc, aimd=None, disp=None, fpath=None, title=None, xtitle=None, ytitle=None):
    ...
    for color_code in ["density", "energy","order"]:
        ...
        if color_code == "density":
            ...
            if aimd is not None and disp is not None:
                ...
                print(f'aimd.energy: {aimd.energy}')
                print(f'disp.energy: {disp.energy}')
        ...
        elif color_code == "order":
            if aimd is not None and disp is not None:
                aimd_order = np.arange(0, len(aimd.energy))
                disp_order = np.arange(0, len(disp.energy))
                viridis.set_over("red")
                scat = ax.scatter(aimd.cn2, aimd.cn1, s=5, c=aimd_order, marker=".", cmap=viridis,
                                  vmin=min(aimd_order), vmax=max(aimd_order) + 10.0)
                scat2 = ax.scatter(disp.cn2, disp.cn1, s=10, c=disp_order, marker="X", cmap=viridis,
                                  vmin=min(disp_order), vmax=max(disp_order) + 10.0)
                cbar = fig.colorbar(scat, ax=ax)
                cbar.set_label(r"Sample ordering", rotation=270, labelpad=20)
                ...
                disp_order = np.arange(0, len(disp.energy))
                viridis.set_over("red")
                scat = ax.scatter(disp.cn2, disp.cn1, s=5, c=disp_order, marker="X", cmap=viridis,
                                  vmin=min(disp_order), vmax=max(disp_order) + 10.0)
                cbar = fig.colorbar(scat, ax=ax)
                cbar.set_label(r"Sample ordering", rotation=270, labelpad=20)

        else:
            raise ValueError(f"color_code={color_code} not recognized!")
