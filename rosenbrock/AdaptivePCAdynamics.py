...
def SumGaussiansPCA(q, qs, eigenvectors, choose_eigenvalue, height, sigma_list):
    ...
    print(f'sigma_list in SumGaussiansPCA: {sigma_list}')
    # V = np.empty((q.shape[0], 1))
    # print(f'eigenvectors.shape: {eigenvectors.shape}')
    # n = sigma_list.shape[0]
    # test_sigma_list = []
    # for i in range(n):
    #     test_sigma_list.append([0.7, 0.7])
    # test_sigma_list = jnp.array(test_sigma_list)
    # print(f'test_sigma_lists.shape: {test_sigma_list.shape}')
    # sigma = 0.7
    V = 0.0
    for i in range(q.shape[0]):
        x_minus_centers = q[i:i + 1] - qs  # N * M
        ...
        # print(f'x_projected_sq.shape: {x_projected_sq.shape}')
        # print(f'x_projected_sq_sum.shape: {x_projected_sq_sum.shape}')
        # sigmas_sq = sigma_list ** 2
        # print(f'sigma_list.shape: {sigma_list.shape}')
        # print(f'sigmas_sq.shape: {sigmas_sq.shape}')
        another_exponent = - x_projected_sq / 2 / sigma_list ** 2
        # print(f'another_exponent.shape: {another_exponent.shape}')
        # print(another_exponent)

        # print(f'sigma: {sigma}')
        # print(f'sigma_list: {sigma_list}')
        # temp = -jnp.expand_dims(x_projected_sq_sum, axis=1) / 2
        # print(f'temp.shape: {temp.shape}')
        # print(temp)
        # exponent = -jnp.expand_dims(x_projected_sq_sum, axis=1) / 2 / sigma ** 2
        # print(f'exponent.shape: {exponent.shape}')
        # print(exponent)
        # exponent_new = -jnp.expand_dims(x_projected_sq_sum, axis=1) / 2 / test_sigma_list ** 2
        # print(f'exponent_new.shape: {exponent_new.shape}')
        # print(exponent_new)

        # another_total_val = jnp.sum(height * jnp.exp(another_exponent))
        # print(f'another_total_val.shape: {another_total_val.shape}')
        # print(f'another_total_val: {another_total_val}')

        # total_val = jnp.sum(height * jnp.exp(exponent), axis=0)
        # print(f'total_val.shape: {total_val.shape}')
        # print(f'total_val: {total_val}')

        # V += jnp.sum(height * jnp.exp(exponent), axis=0)
...
# @jax.jit
def next_step(qnow, qs, eigenvectors, choose_eigenvalue, height, sigma_list, dt=1e-3, beta=1.0, step_cap=0.3):
    seed = int(time.time() * 1e6)  # Generate a seed based on the current time in microseconds
    rng_key = jax.random.PRNGKey(seed)
    rng_key, subkey = jax.random.split(rng_key)

    print(f'sigma_list in next_step: {sigma_list}')
    ...
    if qs is None:
        ...
        step = (- (gradV(qnow) + PCAGradGaussians(qnow, qs, eigenvectors, choose_eigenvalue, height, sigma_list))) * dt
...
def MD(q0, T, Tdeposite, height, sigma, sigma_factor, rng, decay_sigma, dt=1e-3, beta=1.0, n=0, threshold=-1., reset_params=False):
    Nsteps = int(T / dt)
    ...
    for i in tqdm(range(Nsteps)):
        ...
        trajectories[i, :] = q
        q = next_step(q, qs, eigenvectors, choose_eigenvalue, height, sigma_list, dt, beta)
...
def run(filename=None, T=4):
    ...
    trajectory, qs, eigenvectors, eigenvalues, choose_eigenvalue, gradient_directions, sigma_list = MD(q0, T, Tdeposite=Tdeposite, height=height, sigma_factor=sigma_factor, sigma=sigma, decay_sigma=decay_sigma, rng=rng, dt=dt, beta=beta, n=n, threshold=threshold, reset_params=False)  # (steps, bs, dim)
...
if __name__ == '__main__':
    ...
    for i in range(1):
        name = f'PCA_results/run_test_{i+8}.h5'
        run(filename=name, T=3)
