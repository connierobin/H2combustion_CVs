...
def main_plot(potential, potential_name, n, trajectory, qs, eigenvectors_list, choose_eigenvalue_list, gradient_directions, sigma, sigma_list, decay_sigma, height, NstepsDeposite, T, threshold, timings):
    filename = None
    ...
    ax_sigma.axvline(x=timings[0]/100, color='r', linestyle='--', label=f'Upper Right Well at Iter {math.ceil(timings[0]/100)}')
    ...
    for i in range(len(results)):
        ...
        ax_gaussian.set(xlim=(-3, 3), ylim=(-3, 3))
        plt.show()
...
def load_data(filename):
    ...
    # main_plot(pot_fn, parameters['potential'], parameters['n'], trajectory, qs, eigenvectors, choose_eigenvalue, gradient_directions, parameters['sigma'], sigma_list, parameters['decay_sigma'], parameters['height'], NstepsDeposite, parameters['T'], parameters['threshold'], timings)
...
if __name__ == '__main__':
    ...
    results = []
    for i in range(10):
        filename = f'../Trad_results/run_ws_k2_{i+1}.h5'
